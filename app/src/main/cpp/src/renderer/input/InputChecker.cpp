...
namespace renderer {
    ...
    void InputChecker::checkTouchInput() {
        const engine::Input::Button button{engine::Input::Button::TOUCH};

        for (int finger = 0; finger < 3; finger++) {
            engine::Input input{};
            input.touch_finger = finger;
            auto finger_it = _finger_position.find(finger);

            if (sf::Touch::isDown(input.touch_finger)) {
                sf::Vector2<int> touch_position = sf::Touch::getPosition(input.touch_finger);
                input.range_2d = engine::Input::Range2D(
                        {static_cast<float>(touch_position.x), static_cast<float>(touch_position.y)});

                if (finger_it == _finger_position.end()) {
                    input.type = engine::Input::InputType::BUTTONPRESSED;

                    _finger_position.insert({input.touch_finger, input.range_2d});
                } else {
                    input.type = engine::Input::InputType::BUTTONPRESSING;

                    _finger_position[input.touch_finger] = input.range_2d;
                }
                ...
                input.range_2d = _finger_position[input.touch_finger];
                _finger_position.erase(finger_it);
            }
